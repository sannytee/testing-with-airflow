...
RUN set -ex \
    ...
    && pip install pytest \
    && pip install pyasn1 \
    && pip install apache-airflow[postgres,jdbc]==$AIRFLOW_VERSION \
...
RUN cd ${AIRFLOW_HOME}/dags && git clone https://github.com/sannytee/testing-with-airflow.git development
RUN cd ${AIRFLOW_HOME}/dags/development && git checkout -b development
COPY docker_files/dev.conf ${AIRFLOW_HOME}/dags/development/dags/environment.conf
# tst
RUN cd ${AIRFLOW_HOME}/dags && git clone https://github.com/sannytee/testing-with-airflow.git test
RUN cd ${AIRFLOW_HOME}/dags/test && git checkout -b test
COPY docker_files/tst.conf ${AIRFLOW_HOME}/dags/test/dags/environment.conf
#
## acc
RUN cd ${AIRFLOW_HOME}/dags && git clone https://github.com/sannytee/testing-with-airflow.git acceptance
RUN cd ${AIRFLOW_HOME}/dags/acceptance && git checkout -b acceptance
COPY docker_files/acc.conf ${AIRFLOW_HOME}/dags/acceptance/dags/environment.conf
#
## prd
RUN cd ${AIRFLOW_HOME}/dags && git clone https://github.com/sannytee/testing-with-airflow.git production
